...
describe "As a Hiki plugin" do
	...
	it "should convert charset to EUC-JP" do
		target = "\306\374\313\334\270\354"	# `nihongo' in EUC-JP
		begin
			target.force_encoding('EUC-JP')
		rescue NameError
		end
		@plugin.ansi_screen('test_utf8.txt').should include(target)
		@plugin.ansi_screen('test_sjis.txt').should include(target)
		@plugin.ansi_screen('test_euc.txt').should include(target)
	end
end
